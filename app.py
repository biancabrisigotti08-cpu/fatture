...
def parse_pdf_fattura(pdf_bytes):
    ...
    if is_vw_format:
        ...
        # ── Parser Volkswagen — approccio HTML ──

        # Converte ogni pagina PDF in HTML mantenendo la struttura visiva,

        # poi estrae i dati dall'HTML strutturato.
        ...
        class PDFHTMLParser(HTMLParser):
            ...
            def handle_starttag(self, tag, attrs):
                ...
                if tag == 'span':
                    ...
                    if m:
                        ...
                        elif abs(top - self.current_top) > 5:
            ...
            def handle_data(self, data):
                ...
                if self.in_span and data.strip():
        ...
        # Estrai HTML da ogni pagina e raccoglie tutte le righe visive
        ...
        try:
            ...
            with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
                ...
                for page in pdf.pages:
                    ...
                    all_visual_lines.extend(parser.get_lines())

        except Exception as e:

            print(f"Errore HTML parser: {e}")

            # Fallback al testo grezzo

            all_visual_lines = text.split('\n')

        full_text_html = '\n'.join(all_visual_lines)

        print("=== VW HTML TEXT (primi 2000 char) ===")

        print(repr(full_text_html[:2000]))

        print("=======================================")

        # Parser a blocchi sul testo ricostruito dall'HTML

        blocchi_raw = re.split(r'(?=ADDEBITO\s+PENALE\s+PER)', full_text_html, flags=re.IGNORECASE)
        ...
        for blocco_text in blocchi_raw:
            ...
            m_telaio = re.search(r'Tipo\s*dato:\s*TELAIO[\s\n]+Rif\.\s*testo:\s*(\S+)', blocco_text, re.IGNORECASE)

            if not m_telaio:

                m_telaio = re.search(r'TELAIO\s+(\S{17})', blocco_text, re.IGNORECASE)

            telaio_val = m_telaio.group(1).strip() if m_telaio else ""

            # Targa

            m_targa = re.search(r'Tipo\s*dato:\s*TARGA[\s\n]+Rif\.\s*testo:\s*(\S+)', blocco_text, re.IGNORECASE)

            if not m_targa:

                m_targa = re.search(r'TARGA\s+([A-Z]{2}\d{3}[A-Z]{2})', blocco_text, re.IGNORECASE)
